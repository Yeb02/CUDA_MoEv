...
Network::Network(Network* n) {
	...
	for (int i = 0; i < n->complexGenome.size(); i++) {
		complexGenome[i] = std::make_unique<Node_G>(n->complexGenome[i].get());
	}
	for (int i = 0; i < n->complexGenome.size(); i++) {
		// Setting pointers to children.
		// It can only be done once the genome has been constructed:

		complexGenome[i]->children.resize(n->complexGenome[i]->children.size());
		for (int j = 0; j < n->complexGenome[i]->children.size(); j++) {
			complexGenome[i]->children[j] = complexGenome[n->complexGenome[i]->children[j]->position].get();
		}
	...
	topNodeG = std::make_unique<Node_G>(n->topNodeG.get());

	// Setting pointers to children:
	topNodeG->children.resize(n->topNodeG->children.size());
	for (int j = 0; j < n->topNodeG->children.size(); j++) {
		topNodeG->children[j] = complexGenome[n->topNodeG->children[j]->position].get();
	}
	...
	inputSize(inputSize), outputSize(outputSize)
{
	...
	topNodeG = std::make_unique<Node_G>(inputSize, outputSize);
	...
	//complexGenome.emplace_back(new Node_G(15, 15));
	//complexGenome.emplace_back(new Node_G(15, 15));
	//complexGenome.emplace_back(new Node_G(15, 15));
...
void Network::createIDMaps() {
	...
	for (int i = 0; i < complexGenome.size()+1; i++) {
		Node_G* n = i == complexGenome.size() ? topNodeG.get() : complexGenome[i].get();
...
// Network::combine is different depending on the directives used.

#if defined(SPARSE_MUTATION_AND_COMBINATIONS) && defined(COMMON_TOPOLOGY) // too lazy to implement all cases
Network* Network::combine(std::vector<Network*>& parents, std::vector<float>& rawWeights) {

	// ID maps are not used here since the topologies are identical between networks
	...
	Node_G** nodes = new Node_G*[nParents];
	...
	auto copyMatComponents = [](InternalConnexion_G* parentCo, InternalConnexion_G* childCo, int id)
	{
		...
		childCo->D[id] = parentCo->D[id];
		childCo->eta[id] = parentCo->eta[id];
		childCo->alpha[id] = parentCo->alpha[id];
		
#ifndef RANDOM_WB
		childCo->w[id] = parentCo->w[id];
#endif
#ifdef OJA
		childCo->delta[id] = parentCo->delta[id];
#endif
#ifdef CONTINUOUS_LEARNING
		childCo->gamma[id] = parentCo->gamma[id];
#endif
#ifdef GUIDED_MUTATIONS
		childCo->accumulator[id] = parentCo->accumulator[id];
#endif
	};
...
#ifndef RANDOM_WB
		childCo->biases[id] = parentCo->biases[id];
#endif
	...
	auto combineNodes = [&](Node_G* childsNode) 
	{
		for (int j = 0; j < nParents; j++) { connexions[j] = &nodes[j]->toChildren; }
		combineConnexions(&childsNode->toChildren);

		for (int j = 0; j < nParents; j++) { connexions[j] = &nodes[j]->toMemory; }
		combineConnexions(&childsNode->toMemory);

		for (int j = 0; j < nParents; j++) { connexions[j] = &nodes[j]->toOutput; }
		combineConnexions(&childsNode->toOutput);

		for (int j = 0; j < nParents; j++) { connexions[j] = &nodes[j]->toModulation; }
		combineConnexions(&childsNode->toModulation);
	};
	...
	for (int i = 0; i < child->complexGenome.size(); i++) {
		...
		{ 
			nodes[j] = parents[j]->complexGenome[i].get(); 
		}
		combineNodes(child->complexGenome[i].get());
	}
	for (int j = 0; j < nParents; j++)
	{
		nodes[j] = parents[j]->topNodeG.get();
	}
	combineNodes(child->topNodeG.get());
	...
	for (int i = 0; i < child->memoryGenome.size(); i++) {
...
#ifdef QKV_MEMORY
		node->QKV_decay = parents[INT_0X(nParents)]->memoryGenome[i]->QKV_decay;


		// link
		for (int j = 0; j < nParents; j++) {
			connexions[j] = &parents[j]->memoryGenome[i]->link;
		}
		combineConnexions(&node->link);

		//  Q
		int s = node->inputSize * node->kernelDimension;
		for (int j = 0; j < s; j++) {
			Q[j] = parents[INT_0X(nParents)]->memoryGenome[i]->Q[j];
		}
#endif

#ifdef SRWM // TODO not adapted yet.
		...
		for (int l = 0; l < node->nLayers; l++)
		{
			int sW = node->sizes[l] * node->sizes[l + 1];
			for (int j = 0; j < sW; j++)
			{
				node->W0s[l][j] = parents[INT_0X(nParents)]->memoryGenome[i]->W0s[l][j];
			}

			int sB = node->sizes[l + 1];
			for (int j = 0; j < sB; j++)
			{
				node->B0s[l][j] = parents[INT_0X(nParents)]->memoryGenome[i]->B0s[l][j];
			}

		}

		node->learningRate = parents[INT_0X(nParents)]->memoryGenome[i]->learningRate;
#endif
	}
...
Network* Network::combine(std::vector<Network*>& parents, std::vector<float>& rawWeights) {
	...
	Node_G** complexPtrs = new Node_G*[(int)parents.size()];
	...
	for (int i = 0; i < child->complexGenome.size()+1; i++) {
		Node_G * node = i == child->complexGenome.size() ? child->topNodeG.get() : child->complexGenome[i].get();
		...
		for (int j = 1; j < parents.size(); j++) {
			...
			// continue; if different topologies.
			{
				...
				if (
					...
					n->children.size() != node->children.size() ||
				...
				for (int k = 0; k < n->children.size(); k++) {
					if (
						n->children[k]->inputSize != node->children[k]->inputSize ||
						n->children[k]->outputSize != node->children[k]->outputSize
						)
					{
						valid = false;
						break;
		...
		for (int j = 0; j < nValidParents; j++) { connexions[j] = &complexPtrs[j]->toChildren;}
		addConnexions();
	...
	for (int i = 0; i < child->memoryGenome.size(); i++) {
		...
		for (int p = 1; p < nValidParents; p++) {
			for (int _i = 0; _i < 4; _i++) {
				node->SRWM_decay[_i] += weights[p] * log2f(-1.0f / log2f(1.0f - memoryPtrs[p]->SRWM_decay[_i]));
			}
		}
		for (int _i = 0; _i < 4; _i++) {
			node->SRWM_decay[_i] = std::clamp(1.0f - exp2f(-exp2f(-node->SRWM_decay[_i])), .001f, .999f);
...
void Network::zeroAccumulators() {
	...
	for (int i = 0; i < complexGenome.size(); i++) {
		if (complexGenome[i]->phenotypicMultiplicity != 0) {
			complexGenome[i]->toChildren.zeroAccumulator();
...
void Network::createPhenotype() {
	if (topNodeP.get() == NULL) {
		topNodeP.reset(new Node_P(topNodeG.get()));
...
void Network::step(const std::vector<float>& obs) {
	...
	topNodeP->forward(nInferencesOverLifetime == 0);
...
void Network::mutate() {
	...
	{
		float adjustedFMutationP = BASE_MUTATION_P
	...
	*/
	...
	auto computeProbabilities = [this, &probabilities](auto& criterion) {
		float sum = 0.0f;
		for (int i = 0; i < complexGenome.size() + 1; i++) {
			Node_G* n = i != complexGenome.size() ? complexGenome[i].get() : topNodeG.get();
	...
	// Adding child nodes. 
	{
		...
		if (nStructuralMutations > 0) {
			auto criterion = [](Node_G* n) {
				...
				p *= (float)(MAX_COMPLEX_CHILDREN_PER_COMPLEX - n->children.size())/ (float)MAX_COMPLEX_CHILDREN_PER_COMPLEX;
			...
			for (int _unused = 0; _unused < nStructuralMutations; _unused++)
			{
				...
				for (int i = 0; i < complexGenome.size(); i++) {
					...
					for (int j = 0; j < complexGenome[i]->children.size(); j++) {
						if (complexGenome[i]->children[j] == parent) {
							hasParentAsChild[i] = 1;
							break;
						}
						if (hasParentAsChild[complexGenome[i]->children[j]->position] == 1) {
							hasParentAsChild[i] = 1;
							break;
						}
					}
	...
	// Replacing child nodes.
	{
		...
		if (nStructuralMutations > 0) {
			...
			for (int _unused = 0; _unused < nStructuralMutations; _unused++)
			{
				...
				// pick a child:
				int inParentID = INT_0X((int)parent->children.size());
				Node_G* toBeReplacedChild = parent->children[inParentID];
				...
				for (int i = 0; i < complexGenome.size(); i++) {
					...
					for (int j = 0; j < complexGenome[i]->children.size(); j++) {
						if (complexGenome[i]->children[j] == toBeReplacedChild) {
							isPotentialReplacement[i] = -1;
							break;
						}
						if (isPotentialReplacement[complexGenome[i]->children[j]->position] == -1) {
							isPotentialReplacement[i] = -1;
							break;
						}
					}
				...
				if (candidates.size() != 0) {
					...
					Node_G* replacement = candidates[rID];
					parent->children[inParentID] = replacement;
	...
	// Duplicating child nodes.
	{
		...
		if (nStructuralMutations > 0) {
			...
			for (int _unused = 0; _unused < nStructuralMutations; _unused++)
			{
				...
				// pick a child
				int inParentID = INT_0X((int)parent->children.size());
				Node_G* clonedNode = parent->children[inParentID];


				Node_G* n = new Node_G(clonedNode);
...
void Network::sortGenome() {
	...
	std::vector<Node_G*> tempStorage(size); 
	for (int i = 0; i < size; i++) {
...
void Network::updatePhenotypicMultiplicities() {
	...
	for (int i = 0; i < topNodeG->children.size(); i++) {
	...
	for (int i = (int)complexGenome.size() - 1; i >= 0; i--) { 
		for (int j = 0; j < complexGenome[i]->children.size(); j++) {
			multiplicities[complexGenome[i]->children[j]->position] += multiplicities[complexGenome[i]->position];
		}
...
void Network::eraseComplexNode(int genomeID) {
	Node_G* erasedNode = complexGenome[genomeID].get();
	...
	for (int i = genomeID + 1; i < complexGenome.size(); i++) {
		...
		{
			for (int j = 0; j < complexGenome[i]->children.size(); j++) {
				if (complexGenome[i]->children[j] == erasedNode) {
...
float Network::getRegularizationLoss() {
	...
	for (int i = 0; i < complexGenome.size() + 1; i++) {
		...
		accumulate(n->toChildren, &amplitudes_G[i], &nParams_G[i]);
		...
		for (int j = 0; j < n->children.size(); j++) {
			nParams_P[i] += nParams_P[n->children[j]->position];
			amplitudes_P[i] += amplitudes_P[n->children[j]->position];
		}
...
void Network::save(std::ofstream& os)
{
	...
	for (int i = 0; i < topNodeG->children.size(); i++) {
		WRITE_4B(topNodeG->children[i]->position, os);
	}
	for (int i = 0; i < topNodeG->memoryChildren.size(); i++) {
	...
	for (int i = 0; i < complexGenome.size(); i++) {
		for (int j = 0; j < complexGenome[i]->children.size(); j++) {
			WRITE_4B(complexGenome[i]->children[j]->position, os);
		}
		for (int j = 0; j < complexGenome[i]->memoryChildren.size(); j++) {
...
Network::Network(std::ifstream& is)
{
	...
	topNodeG = std::make_unique<Node_G>(is);
	topNodeG->phenotypicMultiplicity = 1;
	...
	for (int i = 0; i < complexGenome.size(); i++) {
		complexGenome[i] = std::make_unique<Node_G>(is);
		complexGenome[i]->position = i;
	}
	topNodeG->position = (int)complexGenome.size();
	for (int i = 0; i < complexGenome.size(); i++) {
	...
	for (int i = 0; i < topNodeG->children.size(); i++) {
		READ_4B(_s, is);
		topNodeG->children[i] = complexGenome[_s].get();
	}
	for (int i = 0; i < topNodeG->memoryChildren.size(); i++) {
		READ_4B(_s, is);
	...
	for (int i = 0; i < complexGenome.size(); i++) {
		...
		for (int j = 0; j < complexGenome[i]->children.size(); j++) {
			READ_4B(_s, is);
			complexGenome[i]->children[j] = complexGenome[_s].get();
		}
		for (int j = 0; j < complexGenome[i]->memoryChildren.size(); j++) {
			READ_4B(_s, is);
